...
These will piggyback on the same table. Listing a user's backup codes is a single `Query` with `sk begins_with BACKUP#`.
...
- **Positive — schema simplicity.** Three attribute definitions for the core table (`pk`, `sk`, `gsi1pk`, `gsi1sk`); no migrations required when new item types are added in Phase 6.
...
- Phase 6 will extend the core table with `BackupCode` and `OtpCode` items; no new attribute definitions or GSIs needed.
